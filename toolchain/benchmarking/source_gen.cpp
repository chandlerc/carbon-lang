...
auto SourceGen::ClassGenState::GetValidTypeName() -> llvm::StringRef {
  ...
  for (;;) {
    ...
    // `BuildClassAndTypeNames` caps how many times each class is referenced so
    // that a valid type always remains here, for any shuffle. This check is a
    // defensive guard on that invariant and should never actually fire.
...
auto SourceGen::ClassGenState::BuildClassAndTypeNames(
  ...
  // that the lazy `GetValidTypeName` search can always find a valid type and
  // fully drain the type name pool, for any shuffle.
  //
  ...
  // types in the loops below, leaving the total type count -- and thus the
  // generated byte count -- fully deterministic and independent of the shuffle.
...
auto SourceGen::GenApiFileDenseDecls(int target_lines,
  ...
  // The name pools must also be fully consumed. This is essential for the
  // byte-count to be stable across seeds: every identifier in these pools has a
  // length that is fixed by the (seed-independent) length distribution, so we
  // emit a deterministic total number of bytes only if we consume all of them.
